...
function tryInferGenerationForOthers(
  ...
  return others.map((othersMember) => {
    ...
    const newInfferedGeneration = nonOthersGenerationsPossible.every((gen) => gen == nonOthersGenerationsPossible[0])
    ...
    couplesInCurrentGeneration.forEach((couple) => {
      ...
      const childrenWithTheirCouples = couplesInNextGeneration
        .filter((nextGenFamilies) => {
          return nextGenFamilies.parents.some((nextGenParent) => childrenIds.some((child) => child == nextGenParent))
        })
        .flat()
      ...
      couple.children = uniqueChildren
        ...
        .sort((childWithFamilyA, childWithFamilyB) => childWithFamilyA.parents.localeCompare(childWithFamilyB.parents))
