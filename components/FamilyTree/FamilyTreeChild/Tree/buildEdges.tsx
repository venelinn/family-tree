...
import { InnerFamilyEdgeData, InnerFamilyTypeKey } from '../Family/InnerFamilyEdge'
...
import { CoupleEdgeTypeKey } from '../Family/CoupleEdge'
...
export function buildEdgesFromParentChildrenRelations(
  ...
  const edgesWithGap = preEdges.map((currentEdge) => {
    ...
    const offsetY =
      GENERATION_HEIGHT / 2 + (thisEdgeIndex + (thisEdgeIndex % 2)) * isOddModifer(thisEdgeIndex) * EDGE_YGAP_MODIFIER
